...
export const createColorLevels = baseColor => ({
  ...
  level3: baseColor.setAlpha(0.8).toHslString(),
  level2: baseColor.setAlpha(0.6).toHslString(),
  level1: baseColor.setAlpha(0.4).toHslString(),
})
...
export const GradientForDay = ({ id, day: { date, levels }, theme, level0 }) => {
  ...
  return (
    <linearGradient id={id}>
      {nonzeroLevels.map((themeIdx, gradientIdx) => (
        <Fragment key={gradientIdx}>
          <stop
            offset={getOffset(gradientIdx, numColors)}
            stopColor={theme[themeIdx][`level${levels[themeIdx]}`]}
          />
          <stop
            offset={getOffset(gradientIdx + 1, numColors)}
            stopColor={theme[themeIdx][`level${levels[themeIdx]}`]}
          />
        </Fragment>
      ))}
