    ...
    return (
        <Modal show={show} onHide={handleClose}>
            <Form>
                ...
                <Modal.Body>
                    <Form.Check checked={preferences.mode === CalendarMode.FRIENDS} label={'Include friends'} onChange={evt => setPreference('mode')(evt.target.checked ? CalendarMode.FRIENDS : CalendarMode.USER_ONLY)} />

                    <Form.Check checked={preferences.includeWorkouts} label={'Include workouts'} onChange={evt => setPreference('includeWorkouts')(evt.target.checked)} />

                    {preferences.mode === CalendarMode.USER_ONLY && preferences.includeWorkouts &&
                    <Form.Check checked={preferences.splitWorkouts} label={'Split workouts by type'} onChange={evt => setPreference('splitWorkouts')(evt.target.checked)} />
                    }

                    {preferences.mode === CalendarMode.FRIENDS && <>
                        <Form.Label>Friends to Include</Form.Label>
                        <MultiSelect options={userOptions}
                                     onChange={setPreference('friends')} />
                    </>}
                </Modal.Body>
                <Modal.Footer>
