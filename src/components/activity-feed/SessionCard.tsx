...
export const SessionCardBody = ({ session, gyms, routes }: Props) => {
  ...
  const distanceClimbed =
    gym !== undefined
      ? heightForSession(session.value, routes, gym, [...ALL_STYLES], true)
      : 'Unknown'
  ...
  return (
    <div className="session-card">
      <Container>
        <Row>
          <Col xs={12}>
            Climbing session at <b>{gymName}</b>
        ...
        <Row className="session-summary-row">
          <Col xs={4}>
            <Card>
              <Card.Body>
                <Card.Title>
                  {session.value.endTime ? sessionDuration(session.value) : 'Ongoing'}
                </Card.Title>
...
export const SessionIcon = ({ session }: Pick<Props, 'session'>) =>
  session.value.endTime === undefined ? (
