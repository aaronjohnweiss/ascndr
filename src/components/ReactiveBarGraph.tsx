...
const ReactiveBarGraph = ({
  ...
  const graphHeight =
    GRAPH_BAR_PADDING + GRAPH_BAR_WIDTH * numUsers * categories.length + domainPadding * 2
        ...
        >
          {showLegend && (
            <VictoryLegend
              ...
              data={legendLabels}
              dataComponent={
                isStacked ? <MultiColorPoint legendColors={legendColors} /> : <Point />
              }
            />
          )}
          <VictoryAxis />
          ...
          <VictoryGroup
            ...
            style={{ data: { width: GRAPH_BAR_WIDTH } }}
          >
            {data.map(({ barData, barTotals }, i) => {
              return (
                <VictoryStack
                  key={i}
                  colorScale={allColors[i]}
                ...
                >
                  {barData.map((bars, j) => {
                    ...
                    return <VictoryBar key={j} data={paddedBarData} barWidth={GRAPH_BAR_WIDTH} />
                  })}
                </VictoryStack>
              )
            })}
