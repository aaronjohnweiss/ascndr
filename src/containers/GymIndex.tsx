...
export const GymIndex = () => {
    ...
    gyms.sort((gymA, gymB) => (getLatestSession(sessionsByGym[gymB.key])?.value?.startTime || 0) - (getLatestSession(sessionsByGym[gymA.key])?.value?.startTime || 0))

    return (
        <Fragment>
            {gyms.map((gym) => <GymCard gym={gym} key={gym.key}
                                               sessions={sessionsByGym[gym.key]}/>)}
            <br/>
