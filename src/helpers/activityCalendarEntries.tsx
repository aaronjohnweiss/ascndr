...
export const buildCounts = <T,>({
  data,
  getTime,
  getCount = () => 1,
  getLevel = () => 0,
  cutoffDate,
}: BuildCountCtx<T>): Record<string, Count> =>
  data
    .filter(x => cutoffDate.isBefore(moment(getTime(x))))
    .map(x => ({
...
export const getLevelByPercentile = (data: CountForDate[]) => {
  ...
  const q50 = percentile(allCounts, 0.5)
...
const buildArray = (map: Record<string, Count>): CountForDate[] =>
  Object.entries(map)
...
export const getCalendarData = ({
  ...
  workouts,
  cutoffDate,
}: Omit<Ctx, 'preferences'>): LabeledData[] => {
...
export const getUserData = ({
  ...
  workouts,
  cutoffDate,
}: Pick<Ctx, 'user' | 'preferences' | 'sessions' | 'routes' | 'workouts' | 'cutoffDate'>) => [
...
export const getSessionData = ({
  ...
  routes,
  cutoffDate,
}: Pick<Ctx, 'sessions' | 'routes' | 'cutoffDate'>) => {
...
export const getWorkoutData = ({
  ...
  cutoffDate,
  preferences,
}: Pick<Ctx, 'workouts' | 'cutoffDate' | 'preferences'>) => {
  if (!preferences.includeWorkouts) return []

  const allWorkoutData = excludeEmpty(
